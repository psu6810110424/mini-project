...
const Home = () => {
  ...
  // ---------------------------------------------------------
  // 2. ส่วนฟังก์ชันการทำงาน (Core Functions)
  // ---------------------------------------------------------
  const handleLogout = () => {
    localStorage.clear();
  ...
  const processBooking = async (field: Field, bookingData: any) => {
    try {
      Swal.fire({ title: 'กำลังตรวจสอบความว่าง...', didOpen: () => Swal.showLoading() });
      ...
      if (res.data.available) {
        ...
        navigate('/payment', { state: { field, bookingData: { ...bookingData, hours: h, totalPrice } } });
        Swal.close();
      } else {
    ...
    <div style={{ backgroundColor: '#f4f7fa', minHeight: '100vh', fontFamily: "'Kanit', sans-serif" }}>
      ...
      <header style={{ background: '#2d3748', padding: '60px 20px', textAlign: 'center', color: 'white' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <h1 style={{ fontSize: '2.5rem', letterSpacing: '2px', margin: 0 }}>RECOMMEND</h1>
          <div style={{ width: '60px', height: '4px', background: '#0066FF', margin: '15px auto' }}></div>
          <p style={{ opacity: 0.7, fontSize: '1.1rem' }}>เลือกสนามคุณภาพที่ได้รับการแนะนำจากผู้ใช้งานสูงสุดในสัปดาห์นี้</p>
        </div>
      </header>

      {/* 3.3 รายการสนาม (Marketplace Grid) */}
            ...
            {fields.map(field => (
              <div key={field.id} className="marketplace-card" style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', border: '1px solid #edf2f7' }}>
                <div style={{ height: '200px', position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', top: '12px', left: '12px', background: '#0066FF', color: 'white', padding: '4px 12px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 700 }}>Recommend</div>
                ...
                <div style={{ padding: '20px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '20px', color: '#718096', fontSize: '0.9rem' }}>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Trophy size={14} /> หญ้าเทียมเกรด A</span>
                  </div>
      ...
      <style>{`
        .pay-method { border: 2px solid #edf2f7; padding: 15px; border-radius: 10px; cursor: pointer; text-align: center; }
        .pay-method.selected { border-color: #0066FF; background: #ebf4ff; color: #0066FF; }
